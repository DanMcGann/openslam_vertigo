  ...
  while (!inFile.eof()) {
    ...
    if (type == "VERTEX_SE2") {
      ...
      Matrix informationMatrix =
          Matrix_(3, 3, info[0], info[1], info[2], info[1], info[3], info[4], info[2], info[4], info[5]);
...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()("help", "Show this help message.")(
      ...
      "stop", po::value<int>(&stop)->default_value(-1), "Stop after this many poses.")("verbose,v", "verbose mode")(
      "sigmoid", "Use the sigmoid as the switch function Psi instead of a linear function.")(
      "dogleg", "Use Powell's dogleg method instead of Gauss-Newton.")("qr",
                                                                       "Use QR factorization instead of Cholesky.")(
      "relinSkip", po::value<int>(&isam2Params.relinearizeSkip)->default_value(10),
      "Only relinearize any variables every relinearizeSkip calls to ISAM2::update (default: 10)")(
      "relinThresh", po::value<double>(&relinThresh)->default_value(0.1),
  ...
  if (vm.count("dogleg"))
    isam2Params.optimizationParams = doglegParams;
  else
    isam2Params.optimizationParams = gaussParams;
  ...
  foreach (Pose p, poses) {
    ...
    for (multimap<int, int>::iterator it = ret.first; it != ret.second; it++) {
      ...
      if (!e.switchable && !e.maxMix) {
        ...
        if (!useSigmoid) {
          ...
          boost::shared_ptr<PriorFactor<SwitchVariableLinear> > switchPriorFactor(new PriorFactor<SwitchVariableLinear>(
          ...
          boost::shared_ptr<PriorFactor<SwitchVariableSigmoid> > switchPriorFactor(
              new PriorFactor<SwitchVariableSigmoid>(Symbol('s', switchCounter), SwitchVariableSigmoid(10.0),
                                                     switchPriorModel));
          graph.push_back(switchPriorFactor);
          ...
          SharedNoiseModel odom_model = noiseModel::Gaussian::Covariance(e.covariance);
          boost::shared_ptr<NonlinearFactor> switchableFactor(
              new BetweenFactorSwitchableSigmoid<Pose2>(planarSLAM::PoseKey(e.i), planarSLAM::PoseKey(e.j),
                                                        Symbol('s', switchCounter), Pose2(e.x, e.y, e.th), odom_model));
          graph.push_back(switchableFactor);
        }
        ...
        boost::shared_ptr<NonlinearFactor> maxMixFactor(
            new BetweenFactorMaxMix<Pose2>(planarSLAM::PoseKey(e.i), planarSLAM::PoseKey(e.j), Pose2(e.x, e.y, e.th),
                                           odom_model, null_model, e.weight));
    ...
    if (verbose) {
      ...
      timer.toc("update");
      cout << "cliques: " << result.cliques << "\terr_before: " << *(result.errorBefore)
           << "\terr_after: " << *(result.errorAfter) << "\trelinearized: " << result.variablesRelinearized << endl;
    } else {
      timer.tic("update");
